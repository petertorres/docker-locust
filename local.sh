...
EOF
    if [ -z $IMAGE ]; then
        IMAGE="artifactory.services.dicedev.dhiaws.com:5000/dice/locust-qa"
    fi
    ...
    if $COMPOSE; then
        ...
        docker run -i --rm -v $PWD/reports:/opt/reports -v ~/.aws:/root/.aws \
        ...
        -e SEND_ANONYMOUS_USAGE_INFO=$KPI -e BUILD_URL=$BUILD_URL -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY $IMAGE
    fi
}
